...
export async function configureUpdatesAsync({
  ...
  await modifyAndCommitAsync(
    ...
    {
      startMessage: 'Configuring expo-updates',
      successMessage: 'We configured expo-updates in your project',
      commitMessage: 'Configure expo-updates',
  ...
  await modifyAndCommitAsync(
    async () => {
      ...
      await modifyExpoPlistAsync(projectDir, expoPlist => {
        ...
        if (
          (runtimeVersion &&
            expoPlist[IOSConfig.Updates.Config.RUNTIME_VERSION] === runtimeVersion) ||
    ...
    {
      startMessage: 'Setting runtime version for expo-updates on iOS',
      successMessage: 'We set runtime version for expo-updates on iOS in your project',
      commitMessage: 'Set runtime version for expo-updates on iOS',
      commitSuccessMessage: 'Successfully committed the configuration changes',
      nonInteractive,
    }
  );
...
export async function setUpdatesVersionsAndroidAsync({
  ...
  await modifyAndCommitAsync(
    ...
    {
      startMessage: 'Setting runtime version for expo-updates on Android',
      successMessage: 'We set runtime version for expo-updates on Android in your project',
      commitMessage: 'Set runtime version for expo-updates on Android',
      commitSuccessMessage: 'Successfully committed the configuration changes',
      nonInteractive,
    }
  );
...
async function modifyAndCommitAsync(
  ...
  try {
    ...
    if (err instanceof gitUtils.DirtyGitTreeError) {
      ...
      try {
        await gitUtils.reviewAndCommitChangesAsync(commitMessage, {
          nonInteractive,
        });
